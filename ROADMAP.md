# Roadmap

## MVP

- Local synthetic text intake.
- Heuristic classifier.
- Deadline-risk signal detection.
- Evidence cards.
- Human Review Gate.
- Structured output.
- Synthetic demo.

## Tests

- Review gate external-action block.
- Synthetic-data-only examples.
- Secret-pattern checks.
- No sending/submission/network modules.
- Deadline-risk review requirement.
- Heuristic classifier behavior.
- Evidence card conservative defaults.
- Demo JSON output.

## Governance

- Security policy.
- Disclaimer.
- Abuse prevention.
- Publication policy.
- Threat model.
- Source-available working draft license.
- Protected Core Audit.

## Audit

- Baseline audit.
- Protected Core Audit.
- Final Audit Report.
- Human Review required before release.

## Release Candidate

Release candidate may be prepared only after tests, CI observation or honest CI blocker documentation, audit completion and protected core exclusion.

## Human Approval

Public release remains blocked until `HUMAN_APPROVAL.md` is intentionally created and reviewed by a human.

## Repository Name

The private target repo currently begins with a leading hyphen:

```text
-civic-protection-cell-public
```

Before public release, consider a clean public repository name:

```text
civic-protection-cell-public
```

## Long-Term Improvements

- Better document structure.
- Better Evidence Cards.
- Better deadline-risk detection without final deadline calculation.
- Better local UI.
- Better protected-core templates outside the public layer.
